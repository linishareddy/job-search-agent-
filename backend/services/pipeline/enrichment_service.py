...
from constants.sources import GROQ_ENRICHMENT_CHUNK_SIZE
...
def _to_enriched(job: DeduplicatedJob, groq: dict) -> EnrichedJob:
    ...
    return EnrichedJob(
        ...
        match_reason=groq.get("match_reason"),
        gaps=groq.get("gaps"),
    )
...
async def enrich_batch(
    ...
    """Send ALL pre-scored jobs to Groq for enrichment, in sequential chunks of
    GROQ_ENRICHMENT_CHUNK_SIZE, and return fully populated EnrichedJob objects for
    every job that was successfully enriched. Chunks are sequential (not parallel)
    to stay under Groq's free-tier RPM. A chunk that fails after retries is skipped
    (logged loudly) rather than failing the whole run.
    """
    chunks = [
        jobs[i : i + GROQ_ENRICHMENT_CHUNK_SIZE]
        for i in range(0, len(jobs), GROQ_ENRICHMENT_CHUNK_SIZE)
    ]
    ...
    for chunk_num, chunk in enumerate(chunks):
        ...
        try:
            ...
            logger.error(
                f"Groq enrichment chunk {chunk_num + 1}/{len(chunks)} failed after "
                f"retries: {e} — skipping {len(chunk)} jobs"
            )
            continue

        # idx is local to this chunk (0..len(chunk)-1) — rebuilt fresh per chunk
        groq_by_idx: dict[int, dict] = {r["idx"]: r for r in groq_results if "idx" in r}

        for i, job in enumerate(chunk):
            enriched.append(_to_enriched(job, groq_by_idx.get(i, {})))

    logger.info(f"Enrichment: {len(jobs)} jobs across {len(chunks)} chunk(s) → {len(enriched)} enriched")
    return enriched
