...
class SearchController:
    ...
    async def list_searches(self, page: int = 1, page_size: int = 100) -> tuple[list[SavedSearchResponse], int]:
        searches, total = await self._repo.get_all(page, page_size)
    ...
    async def bulk_delete_searches(self, data: BulkDeleteSearchesRequest) -> BulkDeleteSearchesResponse:
        if data.only_paused:
            search_ids = await self._repo.get_ids_by_active(False)
        else:
            search_ids = data.ids
        ...
        deleted = await self._repo.delete_many(search_ids)
        return BulkDeleteSearchesResponse(deleted=deleted)

    async def trigger_run(self, search_id: uuid.UUID, background_tasks: BackgroundTasks) -> uuid.UUID:
    ...
    async def get_run_status(self, search_id: uuid.UUID) -> RunStatusResponse:
    ...
    async def create_from_text(
        ...
        run_id = await self.trigger_run(search.id, background_tasks) if run_immediately else None
