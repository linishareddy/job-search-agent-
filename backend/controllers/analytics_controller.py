...
class AnalyticsController:
    ...
    async def get_overview(self, active_only: bool = False) -> GlobalAnalytics:
        searches, total_searches = await self._search_repo.get_all(page_size=1000)
        active_count = sum(1 for s in searches if s.is_active)
        paused_count = total_searches - active_count

        unique_jobs = await self._job_repo.get_all_unique_jobs(active_only=active_only)
        market = analytics_service.compute_analytics(unique_jobs)

        total_matches = await self._job_repo.count_matches(active_only=active_only)
        new_7d = await self._job_repo.count_new_matches_since(days=7, active_only=active_only)
        new_24h = await self._job_repo.count_new_matches_since(days=1, active_only=active_only)

        tracker_raw = await self._app_repo.count_by_status()
        ...
        for search_id, name, job_count, new_count in await self._job_repo.get_search_summaries():
            median = None
            if job_count > 0:
